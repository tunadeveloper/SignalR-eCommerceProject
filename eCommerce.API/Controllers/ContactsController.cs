...
namespace eCommerce.API.Controllers
{
    ...
    public class ContactsController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetById(int id)
        {
            ...
            var mapping = _mapper.Map<ResultContactDTO>(value);
            return Ok(mapping);
        }
        ...
        {
            var values = await _service.GetListByFilterAsyncBL(x => x.ContactEmail.Contains(name));
            var mapping = _mapper.Map<List<ResultContactDTO>>(values);
            return Ok(mapping);
        }

        [HttpPost]
        public async Task<IActionResult> Insert(CreateContactDTO dto)
        {
            var mapping = _mapper.Map<Contact>(dto);
